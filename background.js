...
async function initDB() {
  try {
    ...
    console.log(
      "LOG",
      `Database initialized with ${dicts.length} dictionaries`,
    );
  } catch (error) {
  ...
  if (request.action === "lookup") {
      ...
      try {
        ...
        if (request.queryType === "offline") {
          ...
          const selectedDictionaries =
            request.settings?.selectedDictionaries || [];
          const dictionaryOrder =
            request.settings?.dictionaryOrder || selectedDictionaries;
          ...
          if (selectedDictionaries.length === 0) {
            ...
            definition = await db.lookupTermInDictionaries(
              word,
            ...
            console.log(
              "LOG",
              "Selective offline lookup result:",
              word,
              " -> ",
              definition,
            );
          ...
          request.queryType === "web" ||
          request.queryType === "google_translate"
        ) {
          // Web API lookup (including Google Translate preset)
          definition = await performWebLookup(
            word,
            request.settings,
            request.groupId,
          );
          ...
          // AI service lookup
          definition = await performAILookup(
            word,
            request.context || "",
            request.settings,
            request.groupId,
    ...
    (async () => {
      try {
        const db = await getStructuredDB();
        const dicts = await db.getAllDictionaries();
        const totalWords = dicts.reduce(
          (sum, dict) => sum + dict.counts.terms.total,
          0,
        );
    ...
    (async () => {
      try {
        ...
        console.log(
          "BACKGROUND: Received getSuggestions request for word:",
          word,
          "maxResults:",
          maxResults,
          "dictionaries:",
        ...
        const suggestions = await db.getSuggestionsInDictionaries(
          word,
          maxResults,
          selectedDictionaries,
        );
        console.log(
          "BACKGROUND: Sending response with suggestions:",
          suggestions,
        );
    ...
    (async () => {
      try {
        ...
        console.log(
          "BACKGROUND: Received didYouMean request for word:",
          word,
          "nextChars:",
          nextChars,
          "maxResults:",
        ...
        if (!word.trim()) {
          console.log(
            "BACKGROUND: Word is empty, returning empty did-you-mean suggestions",
          );
          sendResponse({ suggestions: [] });
          return;
        }
        ...
        if (!nextChars.trim()) {
          console.log(
            "BACKGROUND: nextChars is empty, returning empty did-you-mean suggestions",
          );
          sendResponse({ suggestions: [] });
          return;
        }
        ...
        const db = await getStructuredDB();
        console.log(
          "BACKGROUND: Calling db.getDidYouMeanSuggestions with word:",
          word,
          "nextChars:",
          nextChars,
          "maxResults:",
        ...
        const suggestions = await db.getDidYouMeanSuggestions(
          word,
          nextChars,
          maxResults,
          selectedDictionaries,
        ...
        console.log(
          "BACKGROUND: Database returned did-you-mean suggestions:",
          suggestions,
        );
        console.log(
          "BACKGROUND: Sending response with did-you-mean suggestions:",
          suggestions,
        );
        sendResponse({ suggestions: suggestions });
      } catch (error) {
        console.error(
          "BACKGROUND: Error getting did-you-mean suggestions:",
          error,
        );
        sendResponse({ suggestions: [] });
      }
    ...
    (async () => {
      try {
        ...
        console.log(
          "BACKGROUND: Received didYouMeanInternal request for word:",
          word,
          "maxResults:",
          maxResults,
          "dictionaries:",
        ...
        if (!word.trim() || word.length < 4) {
          console.log(
            "BACKGROUND: Word is empty or too short, returning empty did-you-mean-internal suggestions",
          );
          sendResponse({ suggestions: [] });
          return;
        }
        ...
        const db = await getStructuredDB();
        console.log(
          "BACKGROUND: Calling db.getDidYouMeanInternalSuggestions with word:",
          word,
          "maxResults:",
          maxResults,
          "dictionaries:",
        ...
        const suggestions = await db.getDidYouMeanInternalSuggestions(
          word,
          maxResults,
          selectedDictionaries,
        );
        console.log(
          "BACKGROUND: Database returned did-you-mean-internal suggestions:",
          suggestions,
        );
        console.log(
          "BACKGROUND: Sending response with did-you-mean-internal suggestions:",
          suggestions,
        );
        sendResponse({ suggestions: suggestions });
      } catch (error) {
        console.error(
          "BACKGROUND: Error getting did-you-mean-internal suggestions:",
          error,
        );
        sendResponse({ suggestions: [] });
      }
    ...
    (async () => {
      try {
        ...
        const settings = {
          ...
          maxTokens: 2048,
          prompt:
            "You are a helpful assistant. Answer the following question: {text}",
          sendContext: false,
  ...
  if (contentType && contentType.includes("text/html")) {
    ...
    const relevantWords = words.filter(
      (word) =>
        word.length > 2 &&
        !word.match(
          /^(and|the|for|are|but|not|you|all|can|had|her|was|one|our|out|day|get|has|him|his|how|its|may|new|now|old|see|two|way|who|boy|did|has|let|put|say|she|too|use)$/i,
        ),
    );
...
async function performAILookup(word, context, settings, groupId = null) {
  ...
  let prompt =
    serviceSettings.prompt ||
    "You are a Tutor, give a grammar breakdown for: {text}";
  ...
  switch (serviceSettings.provider) {
    ...
    case "openrouter":
      if (serviceSettings.reasoningEnabled) {
        ...
        rawResponse =
          reasoningSummary + content || "No response from OpenRouter";
      } else {
        rawResponse =
...
async function createFavoritesList(name) {
  ...
  if (
    data.lists.some((list) => list.name.toLowerCase() === name.toLowerCase())
  ) {
